...
function afficherJeux() {
    ...
    function rechercherJeux(query) {
      const jeuxRechercher = jeux.filter(jeu => {
        const titreMinuscules = jeu.titre.toLowerCase();
        const recherceMinuscules = query.toLowerCase();
        return titreMinuscules.includes(recherceMinuscules);
      });
        container.innerHTML = "";

        jeuxRechercher.forEach(jeu => {
            ...
            jeuElement.innerHTML = `
                ...
                <div class="descriptionEtButton">
                    ...
                    <div><button type="button" class="btn btn-success rounded-0">Acheter</button> <i class="glyphicon glyphicon-plus"></i></div>
                </div>
            `;
    ...
    recherche.addEventListener("input", function () {
        rechercherJeux(recherche.value);
    });
